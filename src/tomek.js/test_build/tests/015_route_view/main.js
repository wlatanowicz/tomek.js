...
TTestControl015.prototype.createChildControls = function(){
	var ExpressionContext = this;
	var SourceTemplateControl = this;
	var c144 = new TContent( [] );
	this.addTemplateChildControl( "c144", c144 );
	c144.renderTemplateChildControls = function( placeholder ){
		this._templateControls["c145"].renderContentsInPlaceholder( placeholder );
	};
	var c145 = new TContent( [] );
	c144.addTemplateChildControl( "c145", c145 );
	c145.renderTemplateChildControls = function( placeholder ){
		this._templateControls["c146"].renderContentsInPlaceholder( placeholder );
		this._templateControls["c147"].renderContentsInPlaceholder( placeholder );
		this._templateControls["c148"].renderContentsInPlaceholder( placeholder );
	};
	var c146 = new TRouteView(  {
					"Path" : "/tabs/one"
					}  );
	c145.addTemplateChildControl( "c146", c146 );
	c146.renderTemplateChildControls = function( placeholder ){
		var h_c149 = document.createElement( "div" );
		h_c149.setAttribute( "id", "t1" );
		var t_c150 = document.createTextNode( "tab one" );
		h_c149.appendChild( t_c150 );
		placeholder.appendChild( h_c149 );
	};
	var c147 = new TRouteView(  {
					"Path" : "/tabs/two/{option}"
					}  );
	c145.addTemplateChildControl( "c147", c147 );
	c147.attachEvent( "BecameActive", SourceTemplateControl.tabTwoBecameActive.bind( SourceTemplateControl ) );
	c147.renderTemplateChildControls = function( placeholder ){
		var h_c151 = document.createElement( "div" );
		h_c151.setAttribute( "id", "t2" );
		this._templateControls["c152"].renderContentsInPlaceholder( h_c151 );
		placeholder.appendChild( h_c151 );
	};
	var c152 = new TLiteral(  {
					"ID" : "OptionL"
					}  );
	c147.addTemplateChildControl( "c152", c152 );
	var c148 = new TRouteView(  {
					"Path" : "\n\t\t\t/tabs/three\n\t\t\t/tabs/three/*\n\t\t\t/tabs/trzy\n\t\t"
					}  );
	c145.addTemplateChildControl( "c148", c148 );
	c148.renderTemplateChildControls = function( placeholder ){
		var h_c153 = document.createElement( "div" );
		h_c153.setAttribute( "id", "t3" );
		var t_c154 = document.createTextNode( "tab three" );
		h_c153.appendChild( t_c154 );
		placeholder.appendChild( h_c153 );
	};
};
