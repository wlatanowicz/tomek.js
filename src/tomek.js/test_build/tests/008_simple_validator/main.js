...
TTestControl008.prototype.createChildControls = function(){
	var ExpressionContext = this;
	var SourceTemplateControl = this;
	var c84 = new TContent( [] );
	this.addTemplateChildControl( "c84", c84 );
	c84.renderTemplateChildControls = function( placeholder ){
		this._templateControls["c85"].renderContentsInPlaceholder( placeholder );
	};
	var c85 = new TContent( [] );
	c84.addTemplateChildControl( "c85", c85 );
	c85.renderTemplateChildControls = function( placeholder ){
		this._templateControls["c86"].renderContentsInPlaceholder( placeholder );
		this._templateControls["c87"].renderContentsInPlaceholder( placeholder );
	};
	var c86 = new TValidatedForm(  {
					"ID" : "Form"
					}  );
	c85.addTemplateChildControl( "c86", c86 );
	c86.renderTemplateChildControls = function( placeholder ){
		var h_c88 = document.createElement( "div" );
		this._templateControls["c89"].renderContentsInPlaceholder( h_c88 );
		this._templateControls["c90"].renderContentsInPlaceholder( h_c88 );
		placeholder.appendChild( h_c88 );
	};
	var c89 = new TTextBox(  {
					"ID" : "TB"
					}  );
	c86.addTemplateChildControl( "c89", c89 );
	var c90 = new TRequiredValidator(  {
					"CssClass" : "err",
					"ControlToValidate" : "TB"
					}  );
	c86.addTemplateChildControl( "c90", c90 );
	c90.renderTemplateChildControls = function( placeholder ){
		var t_c91 = document.createTextNode( "\n\t\t\t\tNo text in textbox\n\t\t\t" );
		placeholder.appendChild( t_c91 );
	};
	var c87 = new TButton(  {
					"ID" : "B"
					}  );
	c85.addTemplateChildControl( "c87", c87 );
	c87.attachEvent( "Click", SourceTemplateControl.buttonClicked.bind( SourceTemplateControl ) );
};
