...
TTestControl011.prototype.createChildControls = function(){
	var ExpressionContext = this;
	var SourceTemplateControl = this;
	var c107 = new TContent( [] );
	this.addTemplateChildControl( "c107", c107 );
	c107.renderTemplateChildControls = function( placeholder ){
		this._templateControls["c108"].renderContentsInPlaceholder( placeholder );
	};
	var c108 = new TContent( [] );
	c107.addTemplateChildControl( "c108", c108 );
	c108.renderTemplateChildControls = function( placeholder ){
		this._templateControls["c109"].renderContentsInPlaceholder( placeholder );
	};
	var c109 = new TDropDownList(  {
					"DataSource" : ( new TExpression( function(){ return ( SourceTemplateControl.getDdlOptions() ); }.bind( ExpressionContext ) ) ),
					"DisabledFieldName" : "disabled"
					}  );
	c108.addTemplateChildControl( "c109", c109 );
};
